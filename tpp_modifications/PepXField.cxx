...
Field::getText(int mode, 
  ...
  case Field::general: // general
	...
	if (searchEngine == "MASCOT") {
	  ...
	  string queryfile;
	  queryfile = infoMap["xmlFileName"].substr
	    (0,
	     findRightmostPathSeperator(infoMap["xmlFileName"])+1);

	  queryfile += spectrum.substr(0, spectrum.find('.'));
	  text += "&QueryFile=" + rampConstructInputFileName(queryfile);
	  text += "&QueryScanNum=" + 
	    node->getAttrValue("start_scan");
	  text += "\">" + displayedSpectrumText + "</a>";
	}
	else if (searchEngine == "SEQUEST") {
	  text = "<a target=\"Win1\" href=\"";
	  text += infoMap["cgiBase"] + "sequest-tgz-out.cgi?OutFile=";
	  text += infoMap["xmlFileName"].substr(0, findRightmostPathSeperator(infoMap["xmlFileName"])+1) + "/";
	  text += infoMap["runSummaryBaseName"] + "/" + spectrum + ".out";
	  text += "\">" + displayedSpectrumText + "</a>";
	}
	// else, unspecified search engine.
	// this catches comet / x!comet, for which the link doesn't apply
	else {
	...
	string spectrum=node->getAttrValue("spectrum");
	text = "<a target=\"Win1\" href=\"";
	text += infoMap["cgiBase"] + "plot-msms.cgi?MassType=";
...

	{
	  ...
	  toBool(atoi(infoMap["a-ions"].c_str()), a);
	  toBool(atoi(infoMap["b-ions"].c_str()), b);
	  toBool(atoi(infoMap["c-ions"].c_str()), c);
	  toBool(atoi(infoMap["d-ions"].c_str()), d);
	  toBool(atoi(infoMap["v-ions"].c_str()), v);
	  toBool(atoi(infoMap["w-ions"].c_str()), w);
	  toBool(atoi(infoMap["x-ions"].c_str()), x);
	  toBool(atoi(infoMap["y-ions"].c_str()), y);
	  toBool(atoi(infoMap["z-ions"].c_str()), z);
      ...
      else if (mode == Field::plainText) {
	{
	  text = node->
	    findChild("search_result")->
	    findChild("search_hit", "hit_rank", "1")->getAttrValue("peptide_prev_aa");	
	  text += ".";
  ...
  case Field::peptideProphet:
	...
	text = "<a target=\"Win1\" href=\"";
	text += infoMap["cgiBase"] + "ModelParser.cgi?Xmlfile=";
	text += infoMap["msms_pipeline_analysis[@summary_xml]"];
	text += "&amp;Timestamp=";
	text += infoMap["peptideprophet[@time]"];
	text += "&amp;Spectrum=";
...
Field::searchPeptideText(XMLNodePtr node,
  ...
  vector<string> peptideAAVecMods;
